...
public class SwiftPreparedStatement extends SwiftStatementImpl implements PreparedStatement, SwiftStatement {
    ...
    SwiftPreparedStatement(BaseSwiftConnection connection, String sql, JdbcExecutor query, JdbcExecutor maintain) throws SQLException {
        super(connection, query, maintain);
        // 这里检查是否是select的同时也能检查sql是否合法
        SwiftSqlParseUtil.isSelect(sql);
        this.sql = sql;
        this.values = new ArrayList();
        final Matcher matcher = VALUE_POS_PATTERN.matcher(sql);
        while (matcher.find()) {
